...
export async function appendRegistrationToSheet(registration: Registration) {
  ...
  try {
    ...
    // Previously this failed 100% silently, which made "the sheet isn't
    // updating" impossible to diagnose. Now the real error shows up in
    // Vercel → your project → Deployments → Functions/Logs tab.
    console.error("[Google Sheets] appendRegistrationToSheet failed:", e);
  }
}
...
export async function updateSheetStatus(referenceId: string, status: string) {
  ...
  try {
    ...
    console.error("[Google Sheets] updateSheetStatus failed:", e);
  }
}
